...
So, instead of one GR per ecosystem, we compute a GR time series using a 365-day sliding window. The slope (converted to per-year) gives the instantaneous growth rate at that point in time. This lets us:
...
Then, we compute Cardano's relative growth rate by subtracting the average growth rate of Eth and Sol from Cardano's at each point in time. This is the core metric for a [difference-in-differences](https://en.wikipedia.org/wiki/Difference_in_differences) (DiD) style analysis. If we get:
...
1. **Developer Counts (log scale)** — Raw data as lines with a logarithmic scale to be able to compare them (otherwise Cardano's variance is too small).
2. **Rolling 365-Day GR** — Growth rate over time for each ecosystem.
3. **Cardano Relative GR vs Controls** — A DiD-ready chart to be used as an overall view of our current state and to be used as a baseline to assess this proposal's impact. We'll compare the relative GR before and after implementing this proposal to see the results. If effective, the relative GR should shift upward after that line. The higher the better.

![Developer's growth rate (GR) since May 2020](./assets/total-devs_growth-rate_full.png)

The key graph to look at is the last one. In that graph, having $0$ relative GR means we grow at the same rate as Ethereum and Solana. With the huge difference in the total number of developers between Cardano and the others, **Cardano having $0$ relative growth rate would be very bad**, because it means Ethereum and Solana will keep growing by thousands of developers/year while Cardano grows by around a hundred/year. Over time, the gap between them and Cardano grows, making it harder and harder for us to compete. So **our ideal scenario is a sustained positive relative growth rate (the higher, the better)** to slowly but surely catch up with them over many years. However, not only do we not show a positive, sustained trend, but we also show that, most of the time, we have a **negative relative growth rate!** Except for an outlier peak in mid 2020 and during 2023, not only does the gap between Cardano and competitors increase, but also the speed at which that gap gets bigger increases!

There are some caveats with this metric:
...
So, by itself, it doesn't necessarily suggest that Cardano has a weak DevX. But it's a valuable datapoint to use in conjunction with the ones presented next.
...
We'll use the same methodology we used to analyze whether Cardano has a DevX issue (relative growth rate using the difference-in-differences technique) to analyze these three metrics:
